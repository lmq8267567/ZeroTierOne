...
if [ ! -s "$SVC_PATH" ] ; then
   ...
   if [ "$zerosize" -lt 2 ];then
      ...
      curl -L -k -S -o "/tmp/zerotier/zerotier-one" --connect-timeout 10 --retry 3 "https://fastly.jsdelivr.net/gh/lmq8267/ZeroTierOne@master/install/zerotier-one"
      else
      curl -L -k -S -o "/etc/storage/zerotier-one/zerotier-one" --connect-timeout 10 --retry 3 "https://fastly.jsdelivr.net/gh/lmq8267/ZeroTierOne@master/install/zerotier-one"
   fi
fi
if [ ! -s "$SVC_PATH" ] ; then
