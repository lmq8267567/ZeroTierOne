...
[ ! -s "$SVC_PATH2" ] && [ -s "/etc/storage/zerotier-one/zerotier.tar.gz" ] && cp -f /etc/storage/zerotier-one/zerotier.tar.gz /tmp/zerotier.tar.gz
   ...
   if [ ! -s "$SVC_PATH2" ] || [ ! -s "$SVC_PATH2" ] ; then
       ...
       if [ ! -z "$tag" ] ; then
           ...
           if [ "$zerosize" -lt 2 ];then
               ...
               wgetcurl.sh "/tmp/zerotier.tar.gz" "https://github.com/lmq8267/ZeroTierOne/releases/download/$tag/zerotier.tar.gz" "https://fastly.jsdelivr.net/gh/lmq8267/zerotier@master/install/$tag/zerotier.tar.gz"
               else
	      ...
	      rm -rf /etc/storage/zerotier-one/MD5.txt
              ...
              if [ "$zerosize" -lt 2 ];then
               logger -t "【ZeroTier】" "您的设备/etc/storage空间剩余"$zerosize"M，不足2M，将下载安装包到内存安装"
               [ "$zerosize" -gt 1 ] && logger -t "【ZeroTier】" "可尝试手动上传zerotier.tar.gz和MD5.txt到内部存储/etc/storage/zerotier-one/目录里"
               wgetcurl.sh "/tmp/zerotier.tar.gz" "https://github.com/lmq8267/ZeroTierOne/releases/download/1.10.6/zerotier.tar.gz" "https://fastly.jsdelivr.net/gh/lmq8267/zerotier@master/install/1.10.6/zerotier.tar.gz"
               else
                logger -t "【ZeroTier】" "您的设备/etc/storage空间充足:"$zerosize"M，将下载安装包到内部存储"
        ...
        [ ! -s "$SVC_PATH2" ] && [ -s "/etc/storage/zerotier-one/zerotier.tar.gz" ] && cp -f /etc/storage/zerotier-one/zerotier.tar.gz "$SVC_PATH2"
    fi
