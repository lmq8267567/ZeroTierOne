...
if [ -z "$curltest" ] || [ ! -s "`which curl`" ] ; then
   ...
   ztag="$( wget -T 5 -t 3 --user-agent "$user_agent" --max-redirect=0 --output-document=-  https://api.github.com/repos/zerotier/ZeroTierOne/releases/latest  2>&1 | grep 'tag_name' | cut -d\" -f4 )"
   ...
   [ -z "$ztag" ] && ztag="$( wget -T 5 -t 3 --user-agent "$user_agent" --quiet --output-document=-  https://api.github.com/repos/zerotier/ZeroTierOne/releases/latest  2>&1 | grep 'tag_name' | cut -d\" -f4 )"
    else
    ...
    ztag="$( curl --connect-timeout 3 --user-agent "$user_agent"  https://api.github.com/repos/zerotier/ZeroTierOne/releases/latest  2>&1 | grep 'tag_name' | cut -d\" -f4 )"
    ...
    [ -z "$ztag" ] && ztag="$( curl -L --connect-timeout 3 --user-agent "$user_agent" -s  https://api.github.com/repos/zerotier/ZeroTierOne/releases/latest  2>&1 | grep 'tag_name' | cut -d\" -f4 )"
fi
if [ ! -s "$SVC_PATH" ] ; then
   ...
   if [ ! -e "$SVC_PATH2" ] || [ ! -s "$SVC_PATH2" ] ; then
       ...
       if [ ! -z "$tag" ] ; then
	      ...
	      rm -rf /etc/storage/zerotier-one/MD5.txt
              wgetcurl.sh "/etc/storage/zerotier-one/MD5.txt" "https://github.com/lmq8267/ZeroTierOne/releases/download/1.10.6/MD5.txt"
              if [ "$zerosize" -lt 2 ];then
               logger -t "【ZeroTier】" "您的设备/etc/storage空间剩余"$zerosize"M，不足2M，将下载安装包到内存安装"
               wgetcurl.sh "/tmp/zerotier.tar.gz" "https://github.com/lmq8267/ZeroTierOne/releases/download/1.10.6/zerotier.tar.gz"
               else
                logger -t "【ZeroTier】" "您的设备/etc/storage空间充足:"$zerosize"M，将下载安装包到内部存储"
                wgetcurl.sh "/etc/storage/zerotier-one/zerotier.tar.gz" "https://github.com/lmq8267/ZeroTierOne/releases/download/1.10.6/zerotier.tar.gz"
              fi
        fi
 ...
 if [ -f "$SVC_PATH" ] ; then
      ...
      if [ ! -z "$ztag" ] ; then
       if [ "$ztag"x != "$zerotier_v"x ] ; then
          logger -t "【ZeroTier】" "检测到GitHub已发布最新版本zerotier_v$ztag"
	  /etc/storage/script/Sh45_wx_send.sh send_message "【ZeroTier版本更新】" "GitHub已发布最新v$ztag版本"
       fi
      fi
  fi
...
sleep 20
[ -n "$ip11" ] && ping_zero1=$(ping -4 $ip11.1 -c 3 -w 5 -q)
[ -n "$ip22" ] && ping_zero2=$(ping -4 $ip22.1 -c 3 -w 5 -q)
[ -n "$ip33" ] && ping_zero3=$(ping -4 $ip33.1 -c 3 -w 5 -q)
[ -n "$ip44" ] && ping_zero4=$(ping -4 $ip44.1 -c 3 -w 5 -q)
[ -n "$ip55" ] && ping_zero5=$(ping -4 $ip55.1 -c 3 -w 5 -q)
